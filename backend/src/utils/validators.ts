import { ensureDir } from "@std/fs";
import { join } from "@std/path";
import logger from "./logger.ts";
import { throwFatalError } from "./error-handler.ts";

export function validateEnv(): void {
  const required = ["ANTHROPIC_API_KEY", "BRAVE_API_KEY"];
  const missing: string[] = [];

  for (const key of required) {
    if (!Deno.env.get(key)) missing.push(key);
  }

  if (missing.length > 0) {
  const msg =
      "Missing required environment variables:\n" +
      missing.map((v) => `  • ${v}`).join("\n") +
      "\n\nSetup:\n" +
      "1. Copy .env.example to .env\n" +
      "2. Get Anthropic API key: https://console.anthropic.com/\n" +
      "3. Get Brave API key and set BRAVE_API_KEY\n" +
      "4. Add keys to .env file";

    throwFatalError(msg, "INVALID_ENV");
  }
}

export async function ensureDataDirectories(): Promise<void> {
  const base = Deno.cwd();

  const dirs = [
    join(base, "data"),
    join(base, "data", "reports"),
    // checkpoints intentionally omitted
    join(base, "data", "jobs"),
    join(base, "data", "uploads"),
  ];

  for (const dir of dirs) {
    try {
      await ensureDir(dir);
      logger.debug(`Directory ready: ${dir}`);
    } catch {
      throwFatalError(`Failed to create directory: ${dir}`, "DIR_CREATE_FAIL");
    }
  }
}

// legacy ticker validator left to avoid import errors; not used in job flow
export function validateTicker(input: string): string {
  const ticker = input.trim().toUpperCase();
  if (!/^[A-Z0-9.-]{1,10}$/.test(ticker)) {
    throwFatalError(`Invalid ticker: ${input}`, "INVALID_TICKER");
  }
  return ticker;
}
